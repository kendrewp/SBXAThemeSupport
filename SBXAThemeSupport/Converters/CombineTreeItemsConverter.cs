// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CombineTreeItemsConverter.cs" company="Ruf Informatik AG">
...
namespace SBXAThemeSupport.Converters
{
    ...
    public class CombineTreeItemsConverter : IMultiValueConverter
    {
        ...
        {
            var newCollection = new ObservableCollection<ITreeItem>();
            try
            {
                ...
                foreach (var sourceColl in values)
                {
                    ...
                    var parentTreeItem = new TreeItem(string.Empty)
                                             {
                                                 Description =
                                                     string.IsNullOrEmpty(descriptions[collCtr])
                                                         ? string.Empty
                                                         : descriptions[collCtr]
                                             };

                    parentTreeItem.Children = sourceColl as IEnumerable;
                    // must assign it here otherwise when the children are added they are not updated.
                    collCtr++;
