    ...
    public class ScreenProcessDescription : DefinitionDescription
    {
            ...
            : this(fileName, name, expression)
        {
            if (!string.IsNullOrWhiteSpace(definition.Extract(5, 1).Value))
            {
                ...
                this.ProcessCollection.Add(new ProcessCall { Description = "Dictionary", ProcessDescription = new DictionaryDescription(this.Dictionary) });
            }
            if (!string.IsNullOrWhiteSpace(definition.Extract(6).Value))
            {
                ...
                if (!string.IsNullOrEmpty(this.ScreenName.Substring(startIndex)) && Utilities.IsNumber(this.ScreenName.Substring(startIndex)))
                {
                    ...
                    SBFile.ReadDictionaryItem(this.Dictionary, prefix + nextScreen, new object[] { nextScreen, prefix, this.Dictionary }, this.ReadLinkedScreenCompleted);
                }
            }
